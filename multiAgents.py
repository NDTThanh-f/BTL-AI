...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        for action in legalActions:
            ...
            score = self.getValue(successor, 1, 0)
            if score > bestScore:
                bestScore = score
                bestAction = action
    ...
    def getValue(self, gameState: GameState, agentIndex, depth):
        ...
        scores = [self.getValue(successor, nextAgent, nextDepth) for successor in successors]
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        for action in legalActions:
            successor = gameState.generateSuccessor(0, action)
            score = self.getValue(successor, 1, 0, alpha, beta)
            if score > bestScore:
                bestScore = score
                bestAction = action
    ...
    def getValue(self, gameState: GameState, agentIndex, depth, alpha, beta):
        ...
        if agentIndex == 0:
            ...
            for action in legalActions:
                ...
                v = max(v, self.getValue(successor, nextAgent, nextDepth, alpha, beta))
                if v > beta:
                    return v
                alpha = max(alpha, v)
            return v
            ...
            for action in legalActions:
                successor = gameState.generateSuccessor(agentIndex, action)
                v = min(v, self.getValue(successor, nextAgent, nextDepth, alpha, beta))
                if v < alpha:
                    return v
                beta = min(beta, v)
            return v
